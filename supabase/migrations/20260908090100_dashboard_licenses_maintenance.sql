-- ---------------------------------------------------------------------------
-- Two more things the dashboard should say out loud.
--
-- The warranty card already answers "what runs out soon?" for hardware. The
-- same question for licences and for servicing had answers, but only if
-- somebody went looking for them — the licence register and the maintenance
-- screen. A renewal nobody opened the screen to see is a renewal that gets
-- missed.
--
-- Neither number is computed here:
--
--   `license_expiry_state()` already decides what "expiring" means (60 days),
--   and it is what paints the badge in the licence list. A second threshold
--   written here would put a card and a list side by side disagreeing about
--   the same licence.
--
--   `maintenance_due_list()` already derives the due date from the LAST
--   SERVICE — the whole point of the rule — falling back to the purchase date
--   for a machine never serviced. Counting rows from it keeps the card and the
--   list on one definition.
--
-- Expired and overdue are kept apart from expiring and due. A licence that ran
-- out last month and one that runs out next month both need action, but only
-- one of them is already costing somebody their tools, and one number hides
-- that.
--
-- Scope: assets are filtered by `p_locations` throughout. LICENCES ARE NOT —
-- there is no location on a licence, and a seat is held by a person, not a
-- site. Everybody sees the same licence count, which is the same thing the
-- licence register already shows them.
-- ---------------------------------------------------------------------------
...
returns jsonb language sql stable security invoker set search_path = public as $$
  select jsonb_build_object(
    ...
    'licensesExpiring', (
      ...
      where l.is_active
        and license_expiry_state(l.expiry_date) = 'expiring'
    ),
    'licensesExpired', (
      select count(*) from licenses l
      where l.is_active
        and license_expiry_state(l.expiry_date) = 'expired'
    ),

    'maintenanceDue', (
      select count(*) from maintenance_due_list(p_locations, 30)
    ),
    'maintenanceOverdue', (
      select count(*) from maintenance_due_list(p_locations, 30) d
      where d.days_late > 0
    ),
