-- ============================================================================
-- CITE Assets — 0066 Importing licences
--
-- The file is one row per SEAT, because that is the shape the real spreadsheet
-- has: 15 licences spread over 71 rows, and only the first row of each licence
-- names the software. A blank `software` means "another seat of the licence
-- above" — the merged cells of the source, written out.
--
-- IDEMPOTENCE is the hard part, and it is solved differently for the two kinds
-- of seat:
--
--   A seat that names an account is identified by (licence, seat_account).
--   Re-importing updates that seat rather than creating a second one.
--
--   A seat with no account has nothing to be identified BY, so the file is read
--   as an assertion about HOW MANY such seats exist. The importer tops up to
--   that number and never deletes. Re-importing the same file finds the count
--   already satisfied and adds nothing.
--
-- Neither path ever removes a seat or empties a holder — an import can add and
-- correct, never take away. The same rule as import_accounts, for the same
-- reason: a monthly re-import that quietly unassigned somebody would be damage
-- nobody notices until the licence is needed.
-- ============================================================================

-- Excel writes dates as a serial from 1899-12-30; the sheet also carries '-'
-- and real ISO dates in the same column. One parser, so every caller agrees.
create or replace function license_parse_date(p_value text)
...
begin
  ...
  if v ~ '^[0-9]+(\.[0-9]+)?$' then
    n := v::numeric;
    -- Anything below 20000 is far more likely to be a stray number than a date
    -- in 1954, and anything above 80000 is past 2119.
    if n < 20000 or n > 80000 then return null; end if;
    return date '1899-12-30' + (floor(n))::int;
  end if;
...
create or replace function import_licenses(
  p_rows      jsonb,
  p_dry_run   boolean default true,
  p_file_name text    default 'licenses.csv'
...
revoke all on function import_licenses(jsonb, boolean, text) from public, anon, authenticated;
grant execute on function import_licenses(jsonb, boolean, text) to authenticated;
revoke all on function license_parse_date(text) from public, anon;
grant execute on function license_parse_date(text) to authenticated;
