-- ============================================================================
-- CITE Assets — 0054 A department-only file finds the people already there
--
-- THE BUG
-- -------
-- Migration 0053 let a second, smaller file set departments. It did not work.
--
-- A row with no employee ID fell back to matching on name AND company. The
-- department file carries neither an ID nor a company, so v_company was null
-- and the lookup asked for somebody whose company is ALSO null. Everybody
-- imported from Odoo has one, so nothing matched and every row was inserted as
-- a brand new person — silently doubling the register.
--
-- Caught by running the two passes against the real 527-row export rather than
-- by reading the code, which is the only reason it was caught at all.
--
-- THE RULE NOW
-- ------------
--   employee ID present      -> match on it
--   no ID, company present   -> match on name + company
--   neither                  -> match on name alone, but ONLY when that name
--                               belongs to exactly one person
--
-- The last case is the new one. Name alone is safe precisely when it is
-- unique; when it is not, guessing would rewrite the wrong person's record, so
-- the row is refused with a message saying which column would fix it. The real
-- export has six repeated first names, so this is not hypothetical.
--
