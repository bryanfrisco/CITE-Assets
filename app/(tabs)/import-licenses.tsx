/**
 * Import licences from the software licensing spreadsheet.
 *
 * The same three steps as the other two importers — template, see what will
 * happen, commit — because it is the same job.
 *
 * What is different here is the unit. The file is one row per SEAT: fifteen
 * licences arrive as seventy-one rows, and only the first row of each licence
 * names the software. So the preview reports BOTH numbers. Saying "15" over a
 * file of 71 lines would read like something had been dropped.
 *
 * As in the other importers, a preview that finds problems does not fire
 * Import straight away — a sheet names them first. On a clean file that sheet
 * never appears, because a confirmation that always appears is one nobody
 * reads.
 */
...
export default function ImportLicensesScreen() {
  ...
  const [preview, setPreview] = useState<LicenseImportResult | null>(null);
  ...
  const history = useQuery({
    queryKey: ['importHistory', 'licenses'],
    queryFn: () => fetchImportHistory('licenses'),
  });
  ...
  const pick = useMutation({
    mutationFn: async () => {
      ...
      const parsed = parseImportCsv(text, LICENSE_IMPORT);

      if (parsed.missingRequired.length > 0) {
        throw new Error('That file has no Software column — download the template');
      }
      ...
      const result = await importLicenses(parsed.rows, true, chosen.name);
  ...
  const commit = useMutation({
    mutationFn: () => importLicenses(rows!, false, fileName ?? 'licenses.csv'),
    onSuccess: (result) => {
      toast(
        `${result.created} licence${result.created === 1 ? '' : 's'} · ${result.seats} seat${
          result.seats === 1 ? '' : 's'
        }`,
      );
      void queryClient.invalidateQueries({ queryKey: ['importHistory', 'licenses'] });
  ...
  const counts = useMemo(
    () =>
      preview
        ? ([
            ['Licences', preview.created],
            ['Already there', preview.updated],
            ['Seats', preview.seats],
  ...
  if (!can('account.manage')) {
    return (
      <Screen>
        <EmptyState
          title="Not available"
          description="Importing licences is a Super Admin task."
          actionLabel="Back"
  ...
  return (
    <Screen>
      ...
      <Text style={[t.type.screenTitle, { color: t.color.text }]}>Import licenses</Text>
      ...
      <Card padding={15} title="1 · The template" style={styles.card}>
        <Text style={[t.type.meta, styles.hint, { color: t.color.sub }]}>
          One row per seat. Leave Software empty to add another seat to the licence on the row above
          — that is how a licence with 28 seats is written. Categories and vendors must already
          exist in Master data.
        </Text>
        <Text style={[t.type.meta, styles.hint, { color: t.color.sub }]}>
          Seats keep whoever already holds them; an import never empties a chair. A holder is
          matched by name, and a name that matches nobody — or more than one person — leaves the
          seat free rather than guessing.
        </Text>
        <Button
      ...
      {preview ? (
        <Card padding={15} title="3 · Import" style={styles.card}>
          <Text style={[t.type.meta, styles.hint, { color: t.color.sub }]}>
            {willWrite > 0
              ? `${preview.created} new licence${preview.created === 1 ? '' : 's'} and ${preview.seats} seat${preview.seats === 1 ? '' : 's'}. Seats keep whoever already holds them — an import never empties a chair.`
